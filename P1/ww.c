...
int wrap(int width, char* buf, int output_fd){
        ...
        for(int i = 0; i < BUFSIZE; i++){
                ...
                {
                        if(i == BUFSIZE - 1){                                               //If we reach the end of the buffer in the middle of a word
                                if (DEBUG) printf("Accessing Overflow Buffer \n");
                ...
                if(found_end == 1){
                        ...
                        if(overflow == 1){                                                  //If global overflow boolean is set to true
                                if (DEBUG) printf ("Writing Overflow\n");
                                ...
                                if (word_len + 1 > width_left){                             //Checks if word PLUS the space after it will fit in the desired width
...
int main(int argc, char **argv)
{
    ...
    while(bytes_read = read(input_fd, buf, BUFSIZE) > 0)    //Continuously refreshes the buffer
    {
        ...
        if (DEBUG) printf("Refreshing Buffer\n");
    }
